...
# 🏗️ سكريبت إعادة تنظيم مشروع Haymanh Success Initiative
# هذا السكريبت ينظم المشروع إلى مجلدات منفصلة

echo "🚀 بدء إعادة تنظيم المشروع..."
...
cp -r haymanh-success/* haymanh-success-project/frontend/ 2>/dev/null || echo "⚠️ تم نسخ ملفات Frontend"

# نقل ملفات Backend (بدون node_modules)
echo "🖥️ نقل ملفات Backend..."
cp -r backend/src haymanh-success-project/backend/ 2>/dev/null || echo "⚠️ تم نسخ ملفات Backend"
cp backend/package.json haymanh-success-project/backend/ 2>/dev/null || echo "⚠️ تم نسخ package.json"
cp backend/tsconfig.json haymanh-success-project/backend/ 2>/dev/null || echo "⚠️ تم نسخ tsconfig.json"
cp backend/README.md haymanh-success-project/backend/ 2>/dev/null || echo "⚠️ تم نسخ README.md"
cp backend/Dockerfile haymanh-success-project/backend/ 2>/dev/null || echo "⚠️ تم نسخ Dockerfile"
cp backend/docker-compose.yml haymanh-success-project/backend/ 2>/dev/null || echo "⚠️ تم نسخ docker-compose.yml"

# إنشاء مجلد uploads في Backend
mkdir -p haymanh-success-project/backend/uploads/{avatars,events,general,posts,programs}

# إنشاء ملفات الإعداد المشتركة
echo "🔗 إنشاء الملفات المشتركة..."

# إنشاء ملف types مشترك
